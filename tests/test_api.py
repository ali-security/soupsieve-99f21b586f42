...
class TestInvalid(util.TestCase):
    ...
    def test_excessive_relative_selectors(self):
        """Test excessive empty selectors in `:has()`."""

        count = 10000
        prefix = 'div:has('

        # Empty selectors are not forgiven in `:has()`, so parsing must fail on the very
        # first empty selector instead of accumulating one empty selector per comma.
        with self.assertRaises(sv.SelectorSyntaxError) as cm:
            sv.compile('{}{})'.format(prefix, "," * count))
        self.assertEqual(cm.exception.col, len(prefix) + 1)
